...
#define LONG_DISTANCE 100
	...
	//define number of top and last riders
...
// fileLoad: load file from data.txt
...
// getCategory: ask the user which category he wants; S, M, or L
...
// checkCategory: check the catogory user selected
...
// getAgeGroup: to get which age group the rider belongs to
...
// convertTime: to convert time to HH:MM
...
// setRaceTime: to store race time
...
// checkWithdraw: to check out whether the rider withdrew from race
...
void displayAllriders(struct RiderInfo* info, int size)
{
	...
	category = getCategory(info, size);
...
// diplayWinners: to display all winners in all categories
